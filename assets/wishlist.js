...
document.addEventListener('DOMContentLoaded', () => {
  window.initWishlistButton = initButtons;
  document.querySelector('#header__wishlist-quantity').textContent = getWishlist().length;
  window.initWishlistButton();
  initGrid();
});

document.addEventListener('shopify-wishlist:updated', (event) => {
  document.querySelector('#header__wishlist-quantity').textContent = event.detail.wishlist.length;
  // [Shopify Wishlist] Wishlist Updated ✅', event.detail.wishlist;
  initGrid();
});

document.addEventListener('shopify-wishlist:init-product-grid', (event) => {
  // '[Shopify Wishlist] Wishlist Product List Loaded ✅', event.detail.wishlist;
  document.querySelector('#header__wishlist-quantity').textContent = event.detail.wishlist.length;
});

document.addEventListener('shopify-wishlist:init-buttons', (event) => {
  // '[Shopify Wishlist] Wishlist Buttons Loaded ✅', event.detail.wishlist;
  document.querySelector('#header__wishlist-quantity').textContent = event.detail.wishlist.length;
});
